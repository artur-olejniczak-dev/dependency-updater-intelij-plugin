...
public class GradleParser {
    ...
    static final Pattern PLUGIN_PATTERN = Pattern.compile("id\\s*\\(?\\s*['\"]([a-zA-Z0-9.\\-_]+)['\"]\\s*\\)?\\s*version\\s*\\(?\\s*['\"]([^'\"]+)['\"]\\s*\\)?");
    ...
    static final Pattern TOML_PLUGIN_PATTERN = Pattern.compile("([a-zA-Z0-9.\\-_]+)\\s*=\\s*\\{\\s*id\\s*=\\s*['\"]([a-zA-Z0-9.\\-_]+)['\"]\\s*,\\s*version\\.ref\\s*=\\s*['\"](.*?)['\"]");
    ...
    public List<Dependency> extractDependencies(Project project) {
        ...
        for (VirtualFile file : gradleFiles) {
            ...
            while (pluginMatcher.find()) {
                ...
                String versionRaw = pluginMatcher.group(2);
                Dependency dep = new Dependency(pluginId, pluginId + ".gradle.plugin", versionRaw);
                dep.setPlugin(true);
    ...
    private void extractFromToml(Project project, List<Dependency> dependencies) {
        ...
        for (VirtualFile toml : tomlFiles) {
            ...
            Matcher pluginMatcher = TOML_PLUGIN_PATTERN.matcher(content);
            while (pluginMatcher.find()) {
                String pluginId = pluginMatcher.group(2);
                String versionRef = pluginMatcher.group(3);
                if (versionsMap.containsKey(versionRef)) {
                    Dependency dep = new Dependency(pluginId, pluginId + ".gradle.plugin", versionsMap.get(versionRef));
                    dep.setVariable(true);
                    dep.setVariableName(versionRef);
                    dep.setPlugin(true);
                    dependencies.add(dep);
        ...
        for (VirtualFile gradleFile : getGradleFiles(project)) {
            modifyFile(gradleFile, content -> {
                ...
                if (dependency.isPlugin()) {
                    String pluginId = dependency.getGroupId();
                    String pluginRegex = "(id\\s*\\(?\\s*['\"]" + Pattern.quote(pluginId) + "['\"]\\s*\\)?\\s*version\\s*\\(?\\s*['\"])" + Pattern.quote(dependency.getCurrentVersion()) + "(['\"]\\s*\\)?)";
                    updated = updated.replaceAll(pluginRegex, "$1" + newVersion + "$2");
                } else {
                    ...
                    updated = updated.replace(searchTarget, replacement);
